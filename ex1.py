...
def param_weights(returns, bm, start, end):
    weights = []
    # TODO: ONLY ONE MONTH?
    # TODO: 120 or 121 ??
    ...
    for i in range(start, end):
        ...
        row_weights = []
        for x in range(10):
            row_weights.append(round((0.1 * (1 + (0.9 * std_return[x]) + (0.8 * std_bm[x]))), 4))
...
w3 = param_weights(returns_sheet, BM_sheet, 120, returns_sheet.shape[0])
w4 = constrained_param_weights(w3)
...
# def max_p3_sr():
#     w3 = param_weights(returns_sheet, BM_sheet, 120, returns_sheet.shape[0])
#     portfolio_returns(w3, returns_sheet)
#     (np.power((1 + np.mean(returns_p3)), 12) - 1) / (np.std(returns_p3) * np.sqrt(12))
